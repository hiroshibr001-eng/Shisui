...
#define WRAP_VERSION "v2.1-force-surface-createinstance"
...
static int has_ext_name_list(uint32_t count, const char* const* names, const char* name) {
...
VKAPI_ATTR VkResult VKAPI_CALL vkCreateInstance(
  ...
  logf_wrap("ENTER vkCreateInstance info=%p instanceOut=%p", (void*)pCreateInfo, (void*)pInstance);
  ...
  if (!pCreateInfo) {
    VkResult r0 = g_real_vkCreateInstance(pCreateInfo, pAllocator, pInstance);
    logf_wrap("vkCreateInstance passthrough-null result=%d instance=%p", (int)r0, pInstance ? (void*)*pInstance : NULL);
    return r0;
  }

  for (uint32_t i = 0; i < pCreateInfo->enabledExtensionCount; i++) {
    const char* n = (pCreateInfo->ppEnabledExtensionNames && pCreateInfo->ppEnabledExtensionNames[i])
      ? pCreateInfo->ppEnabledExtensionNames[i] : "(null)";
    logf_wrap("vkCreateInstance requested_ext[%u]=%s", i, n);
  }

  uint32_t extra = 0;
  for (uint32_t i = 0; i < g_force_instance_ext_count; i++) {
    if (!has_ext_name_list(pCreateInfo->enabledExtensionCount, pCreateInfo->ppEnabledExtensionNames, g_force_instance_exts[i]))
      extra++;
  }

  VkInstanceCreateInfo patched = *pCreateInfo;
  const char** names = NULL;

  if (extra > 0) {
    uint32_t oldCount = pCreateInfo->enabledExtensionCount;
    uint32_t newCount = oldCount + extra;
    names = (const char**)calloc(newCount, sizeof(char*));
    if (!names) {
      logf_wrap("vkCreateInstance calloc failed extra=%u", extra);
      return VK_ERROR_OUT_OF_HOST_MEMORY;
    }

    for (uint32_t i = 0; i < oldCount; i++)
      names[i] = pCreateInfo->ppEnabledExtensionNames ? pCreateInfo->ppEnabledExtensionNames[i] : NULL;

    uint32_t w = oldCount;
    for (uint32_t i = 0; i < g_force_instance_ext_count; i++) {
      if (!has_ext_name_list(oldCount, pCreateInfo->ppEnabledExtensionNames, g_force_instance_exts[i])) {
        names[w++] = g_force_instance_exts[i];
        logf_wrap("PATCH APPLIED: vkCreateInstance injected extension %s", g_force_instance_exts[i]);
      }
    }

    patched.enabledExtensionCount = newCount;
    patched.ppEnabledExtensionNames = names;
    logf_wrap("PATCH APPLIED: vkCreateInstance extension count old=%u new=%u", oldCount, newCount);
  } else {
    logf_wrap("vkCreateInstance no extension injection needed count=%u", pCreateInfo->enabledExtensionCount);
  }

  VkResult r = g_real_vkCreateInstance(&patched, pAllocator, pInstance);
